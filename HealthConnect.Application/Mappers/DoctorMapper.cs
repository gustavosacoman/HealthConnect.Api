...
public class DoctorMapper : Profile
{
    public DoctorMapper()
    {
        this.CreateMap<Doctor, DoctorSummaryDto>()
            ...
            .ForMember(dest => dest.CRM, opt => opt.MapFrom(opt => opt.DoctorCRMs.FirstOrDefault().CRMNumber))
            .ForMember(dest => dest.State, opt => opt.MapFrom(opt => opt.DoctorCRMs.FirstOrDefault().State))
